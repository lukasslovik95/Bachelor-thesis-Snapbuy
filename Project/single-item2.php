...
<main>
  ...
  <div class="modal fade" id="loginModal" tabindex="-1" role="dialog" aria-labelledby="loginModalTitle" aria-hidden="true">
	...
	<?php
		...
		$ID = $_REQUEST['ID'];
		$ShowBuyNow = mysqli_query($conn, "SELECT * FROM auctions WHERE ID='$ID'");
                ...
                <div class="d-flex align-items-center justify-content-between flex-column flex-sm-row">
                    <div class="order-1 mb-20 mb-sm-0">
						...
						<?php
							...
							if (!empty($user)):
							echo
							'<form method="POST" action="RaiseAuction.php">
								<td><input type="number" name="ProposedPrice" min='.$Price.'></td>
								<td><input type="hidden" name="ID" value='.$row['ID'].'></td>
								<td><input type="hidden" name="User" value='.$user['email'].'></td>
								<td><input type="submit" name="add" value="Podbij"/></td>
							</form>';
							else:

							echo '<h3>Zaloguj się aby zalicytować</h3>';
